...
Blip_Buffer::blargg_err_t Blip_Buffer::set_sample_rate( long new_rate, int msec )
{
	...
	if ( msec != blip_max_length )
	{
		...
		if ( s < new_size )
			...
			assert( 0 ); // fails if requested blipBuffer length exceeds limit
	}
...
{
	offset_ += t * factor_;
	assert( samples_avail() <= (long) buffer_size_ ); // time outside blipBuffer length
}
...
long Blip_Buffer::read_samples( blip_sample_t* out, long max_samples, int stereo )
{
	...
	if ( count )
	{
		...
		int const blip_bass_shift = this->bass_shift;
